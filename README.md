...
- **终端管理**：独立页签实时查看每个托管工位的终端输出（命令、文件改动、工具调用、最终回复逐行滚动），支持一键终止正在运行的执行；每个终端底部有直连输入行（❯），内容**原样透传**进底层会话用于精细化调整，回复以【直连回复】回传到群里。
- **本机终端**：「本机终端」页签内可直接新开 Windows 终端（PowerShell / CMD / pwsh，可指定启动目录），xterm.js 全功能终端仿真（颜色、TUI、快捷键），后端 ConPTY 伪终端 + WebSocket 双向流；会话保存在 hub 里，切页/刷新自动重连并回放缓冲，桌面客户端与网页通用。
- **重启不丢活**：消息表即持久队列——hub 重启后自动把托管员工的积压未读重新派发（单条保持原文，多条合并成一轮）；手工会话的未读保留待其自读。
